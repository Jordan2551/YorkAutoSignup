...
public class SeleniumHelper {
	...
	public void signIn(String SignInUsername, String SignInPassword) {
		...
		Select academicSession = new Select(driver.findElement(By.name("5.5.1.27.1.11.0")));
		WebElement academicSessionSubmit = driver.findElement(By.name("5.5.1.27.1.13"));
		academicSession.selectByVisibleText("FALL/WINTER 2019-2020 UNDERGRADUATE STUDENTS");
	...
	public ArrayList<String> add(ArrayList<String> coursesToEnroll){
		...
		for(int i = 0; i < coursesToEnroll.size(); i++) {
			...
			if(coursesToEnroll.get(i).length() > 0) {
				...
				//ADD CONFIRM
				WebElement addConfirm = driver.findElement(By.xpath("//input[@value='Yes']"));
				addConfirm.click();
